...
static char const *const spi_rsrcs[] = { "spi_clk", "spi_miso", "spi_mosi" };
...
struct msm_spi {
	...
	enum msm_spi_mode        last_tx_mode;
	enum msm_spi_mode        last_rx_mode;
	...
	bool			pack_words;
	bool			last_pack_words;
	u32			last_spi_dev_mode;
	u32			last_bpw;
	u32			last_cs_change;
	u32			last_spi_ioc;
	bool			op_mask;
	u32			spi_ioc_set_cs_set;
	u32			spi_ioc_set_cs_unset;
};
/* clang-format on */
...
static inline void msm_spi_set_qup_config(struct msm_spi *dd, int bpw)
{
	...
	if ((dd->last_bpw == bpw) && (dd->tx_mode != SPI_BAM_MODE))
		return;
...
static inline void msm_spi_enable_error_flags(struct msm_spi *dd)
{
	if (dd->qup_ver == SPI_QUP_VERSION_BFAM)
		writel_relaxed(SPI_ERR_CLK_UNDER_RUN_ERR |
